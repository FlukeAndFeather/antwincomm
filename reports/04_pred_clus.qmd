...
We use the gap statistic [@tibshirani2001] to choose the optimal number of clusters ($k$). The gap statistic ($f$) is a goodness of clustering measure. The authors recommended choosing the smallest $k$ such that $f(k) \geq f(k+1) - s_{k+1}$ where $s$ is the standard error of $f$. According to this rule, the predators are best described by two clusters.
...
sightings_gap_df <- as_tibble(sightings_gap$Tab) %>% 
  mutate(
    k = seq_along(logW),
    is_optimal = k == cluster::maxSE(gap, SE.sim, method = "Tibs2001SEmax")
  )
...
Repeat the indicator species analysis and maps with two clusters.

```{r}
#| label: predator_dendro2

# Cut predators into 2 clusters
clust <- cutree(sightings_clust, 2)
g <- split(names(clust), clust)
p <- ggtree(sightings_clust, hang = -1)
pred_clust_mrca <- sapply(g, function(n) MRCA(p, n))
