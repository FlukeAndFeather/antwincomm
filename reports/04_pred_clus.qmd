...
tar_load(c("predators", "predators_stations", 
           "predators_abundant", "sightings_clust", "sightings_mtx", 
           "sightings_dist", "zoop_sf"), 
...
underway_ice <- predators_stations %>% 
  left_join(transmute(predators,
  ...
  summarize(interval_ice_cat = first(ice_cat),
            .groups = "drop_last") %>% 
  summarize(station_ice_cat = mfv(interval_ice_cat),
            n_obs = n(),
            n_mode = sum(interval_ice_cat == station_ice_cat))
  
underway_ice %>% 
  group_by(n_obs, n_mode, station_ice_cat) %>% 
  ...
  scale_color_fermenter(palette = "RdYlBu",
                        breaks = c(1, 2, 3, 5, 8)) +
  coord_fixed() +
  facet_wrap(~ station_ice_cat, ncol = 2) +
  theme_classic()
