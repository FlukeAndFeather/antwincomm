...
Distribution maps of all species in all years, for supplemental.
...
pred_class_df <- as.data.frame(pred_class, xy = TRUE) %>% 
  pivot_longer(-c("x", "y"), 
               ...
               values_to = "count") %>% 
  drop_na(count) %>% 
  mutate(name = map_chr(name, \(n) paste(strwrap(n, 15), collapse = "\n")))

p <- ant_basemap() +
  geom_raster(aes(x, y, fill = count), pred_class_df) +
  geom_sf(data = tar_read("ant_sf", store = here::here("_targets"))) +
  scale_fill_brewer(palette = "RdPu", 
                    na.value = NA, 
                    direction = 1,
...
ggsave("figs/s1.jpg", p, width = 6, height = 8, units = "in", dpi = 600)
```
