...
effort_stars <- effort_sf %>% 
  ...
  rasterize_counts(count_col = "nmi",
                   res = 2.5e4)
map_lim <- st_bbox(c(xmin = -65, xmax = -55, ymin = -65, ymax = -58),
                       crs = "EPSG:4326") %>% 
  project_bbox() %>% 
...
map_lim <- st_bbox(c(xmin = -65, xmax = -55, ymin = -66, ymax = -60),
                       crs = "EPSG:4326") %>% 
  project_bbox() %>% 
  expand_bbox(factor = 1.1)

ant_basemap(map_lim) +
  stars::geom_stars(aes(fill = log(count_norm + 1)), 
                    data = topfive_stars) +
  scale_fill_distiller(palette = "PuRd",
                       direction = 1,
                       na.value = NA) +
...
map_lim <- st_bbox(soi_sf) %>% 
  project_bbox() %>% 
  expand_bbox(factor = 1.2)

ant_basemap(map_lim) +
  stars::geom_stars(aes(fill = log(count_norm + 1)), data = soi_stars) +
  scale_fill_distiller(palette = "PuRd",
                       direction = 1,
                       na.value = NA) +
  scale_x_continuous(breaks = c(-65, -60, -55)) +
